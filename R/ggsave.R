...
plot_dim <- function(dim = c(NA, NA), scale = 1, units = c("in", "cm", "mm"),
	...
	if (any(is.na(dim))) {
		...
		message("ggmagnolia says: saving ", dim_f[1], " x ", dim_f[2], " ", units, " image")
	}
	
	if (limitsize && any(dim >= 50)) {
		stop(sprintf("
      ggmagnolia says: dimensions exceed 50 inches (height and width are specified in '%s' not pixels).
